...
namespace RPG.Stats
{
    public class TraitStore : MonoBehaviour, IModifierProvider, ISaveable, IPredicateEvaluator
    {
        ...
        //Points assigned to the atributes
        Dictionary<Trait, int> assignedPoints = new Dictionary<Trait, int>();

        //Points is assigning to the atributes, reset to 0 when commit
        ...
        public bool? Evaluate(Predication predicate, string[] parameters)
        {
            if (predicate == Predication.MinimumTrait)
            {
